...
#
# Fast setup with minimal prompts - for when you paste user details
#
################################################################################

set -euo pipefail

# Usage: QUICK_REMOTE_ACCESS.sh <username> [ssh-public-key]
# Example: QUICK_REMOTE_ACCESS.sh john "ssh-rsa AAAAB3..."
...
if [ -z "$USERNAME" ]; then
    ...
    echo "Example: $0 john \"ssh-rsa AAAAB3...\""
    exit 1
fi
...
log "Setting up remote access for: $USERNAME"
...
if ! id "$USERNAME" >/dev/null 2>&1; then
    ...
    log "✅ User created - set password with: sudo passwd $USERNAME"
fi
