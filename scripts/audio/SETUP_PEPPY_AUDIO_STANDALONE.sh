#!/bin/bash
# Complete Audio Setup V1.0 - Standalone
# Copy this script to Pi and run: sudo bash SETUP_PEPPY_AUDIO_STANDALONE.sh
...
if grep -q "dtoverlay=vc4-kms-v3d-pi5,noaudio" /boot/firmware/config.txt 2>/dev/null && \
   grep -q "dtparam=audio=off" /boot/firmware/config.txt 2>/dev/null; then
    ...
    warn "Check HDMI audio disabled in config.txt"
fi

# Find AMP100
if ! grep -q "sndrpihifiberry\|HiFiBerry AMP100" /proc/asound/cards 2>/dev/null; then
    error "AMP100 not detected - check hardware and reboot"
    exit 1
fi
...
if [ -f "$MOODE_DB" ]; then
    ...
    sqlite3 "$MOODE_DB" "UPDATE cfg_system SET value='off' WHERE param='camilladsp';" 2>/dev/null
...
EOF
log "✅ ALSA routing configured"

# PeppyMeter config
if [ -f /etc/peppymeter/config.txt ]; then
    sed -i 's/^meter = .*/meter = blue/' /etc/peppymeter/config.txt 2>/dev/null
    sed -i 's/^random.meter.interval = .*/random.meter.interval = 0/' /etc/peppymeter/config.txt 2>/dev/null
    sed -i 's/^meter.folder = .*/meter.folder = 1280x400/' /etc/peppymeter/config.txt 2>/dev/null
    sed -i 's/^screen.width = .*/screen.width = 1280/' /etc/peppymeter/config.txt 2>/dev/null
    sed -i 's/^screen.height = .*/screen.height = 400/' /etc/peppymeter/config.txt 2>/dev/null
