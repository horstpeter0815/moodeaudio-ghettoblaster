#!/bin/bash
# PeppyMeter Screensaver - Correct Version (checks service status directly)
export DISPLAY=:0
...
# Monitor touch using xinput test
monitor_touch() {
    ...
    DISPLAY=:0 xinput --test "$WAVESHARE_ID" 2>/dev/null | while IFS= read -r line; do
        ...
        if [[ "$line" =~ "button press" ]] || [[ "$line" =~ "button release" ]] || [[ "$line" =~ "motion" ]]; then
            ...
            # Check if PeppyMeter is active (check service status directly)
            if systemctl is-active --quiet peppymeter.service; then
