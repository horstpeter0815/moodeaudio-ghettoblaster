...
if (isset($_POST['update_pcm5122_oversampling'])) {
	if (isset($_POST['pcm5122_oversampling']) && $_POST['pcm5122_oversampling'] != ($_SESSION['pcm5122_oversampling'] ?? '')) {
		$filter = $_POST['pcm5122_oversampling'];
		$result = sysCmd("/usr/local/bin/pcm5122-oversampling.sh " . $_SESSION['cardnum'] . " set \"$filter\"");
		if (!empty($result) && strpos($result[0], 'OK') !== false) {
			...
			$_SESSION['notify']['msg'] = 'Oversampling filter set to: ' . $filter;
		} else {
			$_SESSION['notify']['title'] = NOTIFY_TITLE_ALERT;
			$_SESSION['notify']['msg'] = 'Failed to set oversampling filter. Device may not support this feature.';
		}
	}
}
// Ghetto Crew Flat EQ (Factory Settings)
if (isset($_POST['update_flat_eq'])) {
	$enabled = isset($_POST['ghettoblaster_flat_eq']) && $_POST['ghettoblaster_flat_eq'] === '1';
	
	// Apply Flat EQ preset using moOde's EQ system
	require_once __DIR__ . '/inc/eqp.php';
	
	$preset_file = __DIR__ . '/command/ghettoblaster-flat-eq.json';
	if (file_exists($preset_file)) {
		...
		if ($enabled) {
			...
			// Map bands to 12-band EQ (moOde uses specific frequencies)
			...
			sqlUpdate('cfg_system', $dbh, 'ghettoblaster_flat_eq', '1');
			phpSession('write', 'ghettoblaster_flat_eq', '1');
			$_SESSION['notify']['title'] = NOTIFY_TITLE_INFO;
			$_SESSION['notify']['msg'] = 'Flat EQ enabled';
		} else {
			// Disable Flat EQ - set EQ to Off
			$eqp12 = Eqp12($dbh);
			$eqp12->setActivePresetIndex(0); // 0 = Off
			
			sqlUpdate('cfg_system', $dbh, 'ghettoblaster_flat_eq', '0');
			phpSession('write', 'ghettoblaster_flat_eq', '0');
			$_SESSION['notify']['title'] = NOTIFY_TITLE_INFO;
			$_SESSION['notify']['msg'] = 'Flat EQ disabled';
		}
	} else {
		$_SESSION['notify']['title'] = NOTIFY_TITLE_ALERT;
		$_SESSION['notify']['msg'] = 'Flat EQ preset file not found';
	}
}

// Room Correction (Roon-inspiriert)
if (isset($_POST['update_room_correction'])) {
	if (isset($_POST['room_correction_preset']) && $_POST['room_correction_preset'] != ($_SESSION['room_correction_preset'] ?? 'none')) {
		$preset = $_POST['room_correction_preset'];
		if ($preset == 'none') {
			// Disable room correction
			require_once __DIR__ . '/inc/cdsp.php';
			$previous_cdsp = sqlQuery('cfg_system', $dbh, 'camilladsp_before_room_correction', $_SESSION['camilladsp'] ?? 'off');
			
			// Restore previous CamillaDSP config or turn off
			if ($previous_cdsp != 'off' && $previous_cdsp != '__quick_convolution__.yml') {
				$cdsp = new CamillaDsp($previous_cdsp, $_SESSION['cardnum'] ?? 0, $_SESSION['camilladsp_quickconv'] ?? ',,,');
				$cdsp->selectConfig($previous_cdsp);
				$cdsp->reloadConfig();
				sqlUpdate('cfg_system', $dbh, 'camilladsp', $previous_cdsp);
				phpSession('write', 'camilladsp', $previous_cdsp);
			}
			
			sqlUpdate('cfg_system', $dbh, 'room_correction_active', '0');
			sqlUpdate('cfg_system', $dbh, 'room_correction_preset', 'none');
			phpSession('write', 'room_correction_active', '0');
			phpSession('write', 'room_correction_preset', 'none');
			$_SESSION['notify']['title'] = NOTIFY_TITLE_INFO;
			$_SESSION['notify']['msg'] = 'Room Correction disabled';
		} else {
			// Apply room correction preset
			require_once __DIR__ . '/inc/cdsp.php';
			$filter_file = '/var/lib/camilladsp/convolution/' . $preset . '.wav';
			
			if (!file_exists($filter_file)) {
				$_SESSION['notify']['title'] = NOTIFY_TITLE_ALERT;
				$_SESSION['notify']['msg'] = 'Room Correction preset not found: ' . $preset;
			} else {
				// Save current CamillaDSP config before switching
				$current_cdsp = $_SESSION['camilladsp'] ?? 'off';
				if ($current_cdsp != '__quick_convolution__.yml') {
					sqlUpdate('cfg_system', $dbh, 'camilladsp_before_room_correction', $current_cdsp);
				}
				
				// Copy filter to CamillaDSP coeffs directory
				$coeff_file = '/usr/share/camilladsp/coeffs/room_correction_' . $preset . '.wav';
				if (copy($filter_file, $coeff_file)) {
					sysCmd("sudo chmod a+r \"$coeff_file\"");
					
					// Setup Quick Convolution
					$cdsp = new CamillaDsp($current_cdsp, $_SESSION['cardnum'] ?? 0, $_SESSION['camilladsp_quickconv'] ?? ',,,');
					$quick_conv_config = '0,room_correction_' . $preset . '.wav,room_correction_' . $preset . '.wav,WAVE';
					...
					// Switch to Quick Convolution
					$cdsp->selectConfig('__quick_convolution__.yml');
					$cdsp->setPlaybackDevice($_SESSION['cardnum'] ?? 0, $_SESSION['alsa_output_mode'] ?? 'plughw');
					
					// Update CamillaDSP config (handles mixer type switch if needed)
					$cdsp->updCDSPConfig('__quick_convolution__.yml', $current_cdsp, $cdsp);
				...
				sqlUpdate('cfg_system', $dbh, 'room_correction_preset', $preset);
				phpSession('write', 'room_correction_active', '1');
				phpSession('write', 'room_correction_preset', $preset);
				$_SESSION['notify']['title'] = NOTIFY_TITLE_INFO;
				$_SESSION['notify']['msg'] = 'Room Correction preset applied: ' . $preset;
			}
		}
	}
}
...
// PCM5122 Oversampling Filter (initialize)
$_select['pcm5122_oversampling'] = '';
$_pcm5122_oversampling_value = 'Not available';
// Named I2S devices
...
// PCM5122 Oversampling Filter (only for PCM5122-based devices like HiFiBerry AMP100)
$_pcm5122_oversampling_hide = 'hide';
$_pcm5122_oversampling_disable = 'disabled';
if (isI2SDevice($_SESSION['adevname']) && (
	strpos($_SESSION['adevname'], 'HiFiBerry AMP100') !== false ||
	strpos($_SESSION['adevname'], 'HiFiBerry DAC+') !== false ||
	strpos($_SESSION['adevname'], 'pcm5122') !== false
)) {
	// Check if oversampling control exists
	$result = sysCmd("/usr/local/bin/pcm5122-oversampling.sh " . $_SESSION['cardnum'] . " list");
	if (!empty($result) && strpos($result[0], 'ERROR') === false) {
		$_pcm5122_oversampling_hide = '';
		$_pcm5122_oversampling_disable = '';
		// Get available filters
		$filters = explode(' ', $result[0]);
		$currentFilter = $_SESSION['pcm5122_oversampling'] ?? '';
		// If no current filter, try to get from system
		if (empty($currentFilter)) {
			$getResult = sysCmd("/usr/local/bin/pcm5122-oversampling.sh " . $_SESSION['cardnum'] . " get");
			if (!empty($getResult) && strpos($getResult[0], 'ERROR') === false) {
				$currentFilter = trim($getResult[0]);
			}
		}
		...
		foreach ($filters as $filter) {
			...
			if (!empty($filter)) {
				...
				$_select['pcm5122_oversampling'] .= "<option value=\"$filter\"$selected>$filter</option>\n";
			}
		}
		$_pcm5122_oversampling_value = $currentFilter;
	}
}
// Room Correction Preset Dropdown
$_select['room_correction_preset'] = '';
$_room_correction_preset_value = 'None';
$_room_correction_preset = $_SESSION['room_correction_preset'] ?? 'none';
$_select['room_correction_preset'] .= "<option value=\"none\"" . (($_room_correction_preset == 'none') ? ' selected' : '') . ">None</option>\n";
$presets_dir = '/var/lib/camilladsp/convolution/';
if (is_dir($presets_dir)) {
	$presets = glob($presets_dir . '*.wav');
	foreach ($presets as $preset_file) {
		$preset_name = basename($preset_file, '.wav');
		$selected = ($_room_correction_preset == $preset_name) ? ' selected' : '';
		$_select['room_correction_preset'] .= "<option value=\"$preset_name\"$selected>$preset_name</option>\n";
	}
	if ($_room_correction_preset != 'none' && file_exists($presets_dir . $_room_correction_preset . '.wav')) {
		$_room_correction_preset_value = $_room_correction_preset;
	}
}
// Loopback
...
// Ghetto Crew Flat EQ - Get current state
$flat_eq_enabled = sqlQuery("SELECT value FROM cfg_system WHERE param = 'ghettoblaster_flat_eq'", $dbh);
$flat_eq_enabled = isset($flat_eq_enabled[0]['value']) ? ($flat_eq_enabled[0]['value'] === '1') : false;
$_ghettoblaster_flat_eq_checked = $flat_eq_enabled ? 'checked' : '';
