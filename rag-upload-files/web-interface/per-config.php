...
// Only disable the ON button when the other display is active
// The OFF button should always be enabled so user can turn off current display
$_webui_on_disable = $_SESSION['peppy_display'] == '1' ? 'disabled' : '';
$_webui_off_disable = ''; // OFF button always enabled
$_peppy_on_disable = ($_SESSION['local_display'] == '1' || allowPeppyInAlsaChain() == false) ? 'disabled' : '';
$_peppy_off_disable = ''; // OFF button always enabled
...
if ($_SESSION['feat_bitmask'] & FEAT_LOCALDISPLAY) {
	...
	// ON button: disabled if Peppy is active
	$autoClickOn = " onchange=\"autoClick('#btn-set-local-display');\" " . $_webui_on_disable;
	// OFF button: always enabled
	$autoClickOff = " onchange=\"autoClick('#btn-set-local-display');\"";
	$_select['local_display_on']  .= "<input type=\"radio\" name=\"local_display\" id=\"toggle-local-display-1\" value=\"1\" " . (($_SESSION['local_display'] == 1) ? "checked=\"checked\"" : "") . $autoClickOn . ">\n";
	$_select['local_display_off'] .= "<input type=\"radio\" name=\"local_display\" id=\"toggle-local-display-2\" value=\"0\" " . (($_SESSION['local_display'] == 0) ? "checked=\"checked\"" : "") . $autoClickOff . ">\n";
...
if ($_SESSION['feat_bitmask'] & FEAT_PEPPYDISPLAY) {
	...
	# Display on|off
	// ON button: disabled if Local Display is active or Peppy not allowed in ALSA chain
	$autoClickOn = " onchange=\"autoClick('#btn-set-peppy-display');\" " . $_peppy_on_disable;
	// OFF button: always enabled
	$autoClickOff = " onchange=\"autoClick('#btn-set-peppy-display');\"";
	$_select['peppy_display_on']  .= "<input type=\"radio\" name=\"peppy_display\" id=\"toggle-peppy-display-1\" value=\"1\" " . (($_SESSION['peppy_display'] == 1) ? "checked=\"checked\"" : "") . $autoClickOn . ">\n";
	$_select['peppy_display_off'] .= "<input type=\"radio\" name=\"peppy_display\" id=\"toggle-peppy-display-2\" value=\"0\" " . (($_SESSION['peppy_display'] == 0) ? "checked=\"checked\"" : "") . $autoClickOff . ">\n";
	# Display type
