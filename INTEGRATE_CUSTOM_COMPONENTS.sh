...
# SSH Guaranteed Service (GUARANTEED FIX - 9 Sicherheitsebenen)
cp "$COMPONENTS_DIR/services/ssh-guaranteed.service" \
   "$MOODE_SOURCE/lib/systemd/system/" 2>/dev/null && \
   log "✅ ssh-guaranteed.service kopiert" || \
   log "⚠️  ssh-guaranteed.service bereits vorhanden"

# SSH Watchdog Service (kontinuierliche Überwachung)
cp "$COMPONENTS_DIR/services/ssh-watchdog.service" \
   "$MOODE_SOURCE/lib/systemd/system/" 2>/dev/null && \
   log "✅ ssh-watchdog.service kopiert" || \
   log "⚠️  ssh-watchdog.service bereits vorhanden"

# Network Guaranteed Service (GUARANTEED FIX - 4 Fallback-Mechanismen)
cp "$COMPONENTS_DIR/services/network-guaranteed.service" \
   "$MOODE_SOURCE/lib/systemd/system/" 2>/dev/null && \
   log "✅ network-guaranteed.service kopiert" || \
   log "⚠️  network-guaranteed.service bereits vorhanden"
